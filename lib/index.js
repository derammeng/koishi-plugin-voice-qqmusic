...
// 插件名称，用于日志和配置
exports.name = 'voice-qqmusic';
// 插件配置Schema定义
exports.Config = koishi_1.Schema.object({
    groupVoiceEnabled: koishi_1.Schema.boolean()
        .default(true)
        .description('群聊中是否发送语音消息'),
    privateVoiceEnabled: koishi_1.Schema.boolean()
        .default(true)
        .description('私聊中是否发送语音消息'),
    groupPageSize: koishi_1.Schema.number()
        .default(5)
        .min(1)
        .max(10)
        .description('群聊中每页显示的歌曲数量'),
    privatePageSize: koishi_1.Schema.number()
        .default(10)
        .min(1)
        .max(20)
        .description('私聊中每页显示的歌曲数量'),
    maxResults: koishi_1.Schema.number()
        .default(20)
        .min(5)
        .max(50)
        .description('搜索结果最大返回数量'),
    voiceTimeout: koishi_1.Schema.number()
        .default(30)
        .min(10)
        .max(120)
        .description('语音消息超时时间（秒）'),
    showSinger: koishi_1.Schema.boolean()
        .default(true)
        .description('是否显示歌手信息'),
    showAlbum: koishi_1.Schema.boolean()
        .default(true)
 ...
 * @returns 搜索结果列表
 */
async function searchQQMusic(keyword, maxResults) {
    try {
        // QQ音乐搜索API
        ...
        // 解析歌曲列表
        return data.data.song.list.map((item) => ({
            id: String(item.id),
            name: item.name,
            singer: item.singer || [],
            album: item.album,
 ...
 * @returns 播放链接
 */
async function getMusicUrl(songMid) {
    try {
        // 使用QQ音乐歌曲详情API获取播放链接
 ...
 * @returns 格式化后的文本
 */
function formatSongInfo(song, index, config) {
    const parts = [`${index}. ${song.name}`];
    ...
    ctx.middleware(async (session, next) => {
        ...
        try {
            ...
            if (voiceEnabled) {
                try {
                    // 使用 Koishi 的语音消息格式
                    await session.send((0, koishi_1.h)('record', { url: musicUrl }));
                }
                catch (voiceError) {
            ...
            if (song.cover) {
                try {
                    await session.send(koishi_1.h.image(song.cover));
                }
                catch (imageError) {
                    // 图片发送失败时忽略
