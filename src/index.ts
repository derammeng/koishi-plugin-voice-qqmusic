...
{items} &\#x3C;div class=&\#x22;footer&\#x22;&\#x3E;&\#x56DE;&\#x590D;&\#x6570;&\#x5B57; 1-{songs.length} 选择歌曲，回复 0 取消</div>
</div>
</body>
</html>`
}
...
if (!showTimestamp) lyrics = lyrics.replace (//g, '').trim ()
return lyrics
}
return null
} catch (error) {
...
if (isGroupChat && env?.maxDuration > 0 && song.duration > env.maxDuration) return ❌ 歌曲过长（限制 ${formatTime(env.maxDuration)}）
...
// 登录命令
ctx.command ('QQ 音乐 QQ 登录 ', 'QQ 音乐 QQ 登录，仅私聊可用，需要权限 4')
...
// 微信登录命令
ctx.command ('QQ 音乐 微信登录 ', 'QQ 音乐微信登录，仅私聊可用，需要权限 4')
.action (async ({ session }) => {
if (session.guildId) {
return '❌ 该命令仅支持私聊使用，请在私聊中发送该命令 '
}
...
// 退出登录命令
ctx.command ('QQ 音乐 退出登录 ', ' 退出 QQ 音乐登录 ')
.action (async ({ session }) => {
...
const userSession = ctx.qqMusic.getUserSession (session.userId)
if (!userSession) {
return '❌ 你还未登录 QQ 音乐，请先在私聊中发送 “QQ 音乐 QQ 登录” 或 “QQ 音乐 微信登录” 进行登录 '
}
...
return ['📊 系统状态 ', 缓存文件: ${files.length} 个, 缓存大小: ${(size / 1024 / 1024).toFixed(1)} MB, 群聊: ${config.group?.enabled ? '✅' : '❌'}, 私聊: ${config.private?.enabled ? '✅' : '❌'}].join ('\n')
} catch (error) {
return '❌ 读取状态失败 '
}
})
